...
WRITE_FILENAME = 'ble_rssi_data_avg_d.csv'
...
def calculate_average_rssi(data, target_devices):
    ...
    for key in rssi_values:
        x, y, direction = key
        avg_entry = {'x': x, 'y': y, 'direction': direction}
        for device in target_devices:
...
def write_average_rssi_to_csv(averages, target_devices, filename):
    # จัดเรียงข้อมูลตาม y, x, และ direction ตามลำดับทิศทางที่กำหนด
    sorted_averages = sorted(averages, key=lambda entry: (int(entry['y']), int(entry['x']), get_direction_index(entry['direction'])))
    
    # เขียนข้อมูลลงในไฟล์ CSV
    with open(filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        header = ["No", "x", "y", "direction"] + target_devices
        writer.writerow(header)
        
        current_no = 0
        
        for avg_entry in sorted_averages:
            if avg_entry['direction'] == "N":
                current_no += 1
            row = [current_no, avg_entry['x'], avg_entry['y'], avg_entry['direction']]
            for device in target_devices:
