...
def find_closest_match(avg_rssi, csv_data, k=4):
    ...
    for entry in csv_data:
        ...
        closest_matches.append(((entry['x'], entry['y']), entry['direction'], total_diff))
...
def calculate_knn_average_position(closest_matches):
    sum_x, sum_y = 0, 0
    for (x, y), _, _ in closest_matches:
        sum_x += int(x)
        sum_y += int(y)
...
def calculate_weighted_average_position(closest_matches):
    ...
    for (x, y), _, diff in closest_matches:
        weight = 1/diff
        sum_x += int(x) * weight
        sum_y += int(y) * weight
        total_weight += weight
...
def train_and_predict_svm(csv_data, avg_rssi):
    ...
    print(f"Best parameters for SVR (x): {svr_x.best_params_}")
    print(f"Best parameters for SVR (y): {svr_y.best_params_}")
